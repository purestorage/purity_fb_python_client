...
class RESTClientObject(object):
    ...
    def request(self, method, url, query_params=None, headers=None,
        ...
        # sometimes we remove our content type to let http libraries auto-populate it along with
        # attributes like boundary and character set, so save it now as we need it to interpret
        # our response
        saved_content_type = headers['Content-Type']
        ...
        try:
            ...
            if method in ['POST', 'PUT', 'PATCH', 'OPTIONS', 'DELETE']:
                ...
                if re.search('json', saved_content_type, re.IGNORECASE):
                    request_body = None
                    if body:
                    ...
                    r = self.pool_manager.request(method, url,
                                                  ...
                                                  timeout=timeout,
                                                  headers=headers)
                elif saved_content_type == 'application/x-www-form-urlencoded':
                    r = self.pool_manager.request(method, url,
                                                  fields=post_params,
                                                  ...
                                                  timeout=timeout,
                                                  headers=headers)
                elif saved_content_type == 'multipart/form-data':
        ...
        if _preload_content:
            ...
            # we need to decode it to string if we're not sending form data (in which case our
            # response is a file) and if we're not Accepting application/octet-stream.
            response_is_definitely_file_data = (saved_content_type == 'multipart/form-data')
            if PY3 and not response_is_definitely_file_data:
                # even if our response isn't DEFINITELY file data, it still COULD be file data
                try:
                    decoded_data = r.data.decode('utf8')
                    r.data = decoded_data
                except:
                    # make sure we don't try to log this data if it's not utf-8 compatible
                    response_is_definitely_file_data = True
            ...
            if not response_is_definitely_file_data:
